...
def generate_dynamic_matchup():
    ...
    if primary_key:
        try:
            ...
            print(f"❌ Primary API error or no credits: {e}")
            
    if not script or len(script) < 5:
        print("⚠️ Utilizing internal layout script templates.")
        if "Jujutsu Kaisen" in title:
            script = [f"What happens if a warrior gets trapped in Jujutsu Kaisen?", "Cursed energy floods the atmosphere instantly.", "Special grade curses launch a massive ambush.", "But a wild god tier counter attack is triggered!", "The jujutsu world has never seen power like this."]
        elif "Domain Expansion" in title:
            script = ["What if characters combined skills with a Domain Expansion?", "The reality barrier completely shatters under pressure.", "An absolute, unavoidable critical hit is locked in.", "Even the strongest anime gods cannot escape this layout.", "This combination completely breaks the scaling charts!"]
        elif "Takes Over!" in title:
            script = ["What if a hero turned completely evil and betrayed everyone?", "A dark cloud falls over the entire universe.", "Only one legendary warrior steps up to stop the destruction.", "The final clash of titans begins right now.", "Comment down below who survives this betrayal!"]
        else:
            script = ["Who actually wins this ultimate deathmatch showdown?", "The attacker brings insane speed blitz potential and cosmic hacks.", "Stop coping! The defender completely outclasses them in raw IQ.", "One definitive strike completely shatters their defense system.", "Debate down below with your real scaling facts!"]
...
def generate_free_voice(text_to_speak):
    print("🎙️ Activating free voice generation...")
